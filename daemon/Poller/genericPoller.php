...
if (isset($cliOptions['i'])) { $iterationCycle=intval($cliOptions['i']); }
if (isset($cliOptions['s'])) { $daemonState=$cliOptions['s']; }
if (empty($iterationCycle)) {
  echo "FATAL: -i is a manditory parameter for your iteration cycle\n";
  exit();
  $iterationCycle=3;  // we will never hit this, duh
}

if (isset($cliOptions['t'])) { $monitorType=$cliOptions['t']; $monitorType=strtolower($monitorType); }
if (empty($monitorType)) { $monitorType="snmp";  }
if (empty($daemonState)) { $daemonState='start'; }

// Start setting your logging values
if ( isset($daemonLog[$monitorType])) {
  $logSeverity = $daemonLog[$monitorType];
}
else {
...
if ( isset($maxWork[$monitorType])) {
  $defaultMaxWork = $maxWork[$monitorType];
}

if ( isset($maxChildren[$monitorType])) {
  $defaultMaxChildren = $maxChildren[$monitorType];
}
